...
class OfficerController extends Controller
{
    ...
    public function index()
    {
        ...
        // Get the tenant owned by the logged-in user
        ...
        $dukas = Duka::where('tenant_id', $tenant->id)->get();

        return view('officers.index', compact('officers', 'tenant', 'dukas'));
    }
   ...
   public function unassign($id)
{
    ...
    // UPDATE the status to inactive (false) instead of deleting
    $officerAssignment->update([
        'status' => false
    ]);
    ...
    return redirect()->back()->with('success', 'Officer has been unassigned (deactivated) successfully. An email notification has been sent.');
}
   ...
   public function store(Request $request)
{
    ...
    // 1. Get the Tenant ID from the logged-in user
    // Note: Assuming your User model has a tenant_id or you are getting the tenant via the user relationship
    $tenant = Auth::user()->tenant;
    $tenantId = $tenant->id;

    // 2. Create/Get the role
    $officerRole = Role::firstOrCreate(['name' => 'officer']);

    // 3. Create the officer user
    $officer = User::create([
        'name'      => $request->name,
        'email'     => $request->email,
        'password'  => Hash::make($request->password),
        'role'      => 'officer',
        'tenant_id' => $tenantId, // Linking officer to the tenant
    ]);

    $officer->assignRole('officer');

    // 4. Assign officer to all Dukas of the tenant
    ...
    return redirect()->route('officers.index')->with('success', 'Officer created and assigned to all shops successfully!');
}
    ...
    public function reassign($id)
{
    ...
    $assignment = TenantOfficer::where('id', $id)
        ->where('tenant_id', $tenantId)
        ->firstOrFail();

    $assignment->update([
        'status' => true,
        // Optional: Update the assigned date to today
        'created_at' => now()
    ]);

    return redirect()->back()->with('success', 'Officer has been reassigned to the duka successfully.');
}
