...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        return DB::transaction(function () use ($request) {
            ...
            return redirect()->route('login')->with('success', 'Welcome to StockflowKP! Your 14-day trial has started.');
        });
    }
    ...
    public function login(Request $request)
    {
        ...
        ->with('info', 'Officer web access is coming soon.');
        }
    ...
    public function registerApi(Request $request)
    {
        ...
        try {
            return DB::transaction(function () use ($request) {
                ...
                // 1. Create the User first (Role: Tenant/Owner)
                // We leave tenant_id null for a split second
                $user = User::create([
                    'name'     => $request->name,
                    'email'    => $request->email,
                    'password' => Hash::make($request->password),
                ...
                // 2. Create the Tenant and link the User ID
                $tenant = Tenant::create([
                    'name'             => $request->business_name,
                    'user_id'          => $user->id, // Now valid!
                    'status'           => 'active',
                    'slug'             => Str::slug($request->business_name) . '-' . Str::random(5),
                    'default_password' => '123456', // Optional: tracking default pass
                ]);

                // 3. Update User with the new Tenant ID
                $user->update(['tenant_id' => $tenant->id]);
