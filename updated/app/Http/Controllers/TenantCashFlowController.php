...
class TenantCashFlowController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Transaction::where('duka_id', $selectedDukaId)
            ->where(function ($q) {
                // Cashflow Report: Only include actual cash in/out.
                // Exclude Sales that are LOANS (Credit sales), as no cash was received yet.
                // Loan Repayments (category 'loan_repayment') will be picked up naturally.
                $q->where('category', '!=', 'sale')
                    ->orWhereNotIn('reference_id', function ($sub) {
                        $sub->select('id')->from('sales')->where('is_loan', true);
                    });
            })
        ...
        if ($request->has('export') && $request->export == 'pdf') {
            $selectedDuka = $dukas->firstWhere('id', $selectedDukaId);
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('tenant.cashflow.pdf', compact(
                'transactions',
                'totalIncome',
                'totalExpense',
                'netCashFlow',
            ...
            return $pdf->download('cashflow_statement.pdf');
        }
    ...
    public function profitAndLoss(Request $request)
    {
        ...
        if ($request->has('export') && $request->export == 'pdf') {
            $selectedDuka = $dukas->firstWhere('id', $selectedDukaId);
            $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('tenant.reports.p_and_l_pdf', compact(
                'totalRevenue',
                'cogs',
                'grossProfit',
                'expenses',
