 ...
 public function officerdashboardinformation(Request $request)
    {
        ...
        if (!$user) {
            Log::warning("Jaribio la kuingia dashboard bila Authorization.");
            return response()->json(['success' => false, 'message' => 'Unauthenticated'], 401);
        }

        Log::info("Fetching dashboard for Officer: {$user->name} (ID: {$user->id})");

        // 2. Tafuta assignment ya duka (Tunaepuka firstOrFail kuzuia 404)
        $assignment = TenantOfficer::where('officer_id', $user->id)
            ->where('status', true)
            ->first();

        if (!$assignment) {
            Log::error("Dashboard Error: No active assignment for Officer ID: {$user->id} (Petson, hakikisha huyu user amepangiwa duka kwenye table ya tenant_officers)");
            return response()->json([
                'success' => false,
                'message' => "Hujapangiwa duka lolote kwa sasa. Tafadhali wasiliana na Admin."
            ], 403);
        }
        ...
        try {
            ...
            // Kupata ID za maduka yote aliyopangiwa huyu mfanyakazi
            ...
            Log::info("Officer {$user->id} assigned to Tenant: {$tenantId}, Duka IDs: " . $dukaIds->implode(', '));

            // 3. Gather Data optimized for SQLite Sync (FIFO & Sales)
