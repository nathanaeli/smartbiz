...
class DukaProducts extends Component
{
    ...
    public $filterStock = ''; // all, in_stock, low_stock, out_of_stock
    public $sortBy = 'id';
    public $sortDir = 'desc';
    ...
    {
        $query = Product::where('duka_id', $this->duka->id)
        ...
        if ($this->filterStock) {
            if ($this->filterStock === 'in_stock') {
                $query->whereHas('stocks', fn($q) => $q->where('duka_id', $this->duka->id)->where('quantity', '>', 0));
            } elseif ($this->filterStock === 'low_stock') {
                $query->whereHas('stocks', fn($q) => $q->where('duka_id', $this->duka->id)->where('quantity', '>', 0)->where('quantity', '<=', 10));
